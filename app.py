...
# -------------------------- NEW FLASK ROUTE --------------------------
# This endpoint will handle the audio file from the browser
...
def transcribe_audio():
    ...
    try:
        # Convert webm/ogg audio from browser to WAV format that the library needs
        audio = AudioSegment.from_file(audio_file, format=audio_file.content_type.split('/')[-1])
        wav_io = io.BytesIO()
        audio.export(wav_io, format="wav")
        wav_io.seek(0)

        # Use the SpeechRecognition library to process the WAV data
        r = sr.Recognizer()
...
def home():
    ...
    # --- (The rest of your home() function remains exactly the same) ---
    if not search:
...
# --- (All your other routes like /job/<id>, /makeJobs, etc. remain the same) ---
# ...
...
def make_jobs():
    global jobs # you need to declare jobs as global to modify it
    global job_embeddings # and job_embeddings too
    if request.method == "POST":
        ...
        print(f"New embedding shape: {job_embeddings.shape}")
